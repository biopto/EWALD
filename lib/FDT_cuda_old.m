...
function [RECON,dxo, nGPi, N_Kspace_xy_padded, KO,KOi, RMAEtab,RRMSEtab,RMADtab,RRMSDtab] = ...
			FDT_cuda_old(SINOamp,SINOph, sino_params, thetay, ... % sinogram
...
relaxLaR = 0.99;%0.99;%1.00, 0.99(best with N_Kspace_z_padded_upsampled > N_Kspace_xy_padded), 0:disable !!!!
...
% GP begins here:
if nGPI_CUDA  % GP/GPSC on CUDA
    
    if nGPi>0; disp('Fourier iterations on CUDA ...'); end
...
%     fullpathToDll    = 'E:\User-Marcin\TT2\CUDA\12\CUDAprocessing\x64\Debug\CUDAprocessing.dll';
    ...
    CUDAprocessingPath = fullfile(lib_folder_path, '\GP_CUDA\CUDAprocessing');
       
    fullpathToHeader = fullfile(CUDAprocessingPath, 'CUDAprocessing.h');
    fullpathToDll = fullfile(CUDAprocessingPath, 'CUDAprocessing.dll'); 
    ...
    error = calllib('CUDAprocessing','setParams',do_TC, do_NNC, n_imm, relaxM, betaM, kn_mean, objshift, dxo, nGPi);
    checkError(error);

    data_forC_1D = convertComplexMatrixForC(n_rec);

    error = calllib('CUDAprocessing','sendData_n_rec', data_forC_1D);
    checkError(error);

    objSupport1D = convertFloatMatrixForC(object_support);

    error = calllib('CUDAprocessing','sendData_objectSupport', objSupport1D);
    checkError(error);
    ...
    KO1D = convertComplexMatrixForC(fftshift(KO));

    error = calllib('CUDAprocessing','sendData_KO_Re_Im_and_EW8bit', KO1D, EW_8bit1D, elements);
    checkError(error);
    ...
    clear objSupport1D EW_8bit1D KO1D p

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% Marcin / CUDA
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
else % use standard impementation of GP
    
    if nGPi>0; disp('Fourier iterations ...'); end
