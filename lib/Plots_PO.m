...
if ~exist('map','var'); map = 'jet'; end

% if all(sino_params(5,:)==2) && ~OCT_TF_shift
if all(sino_params(5,:)==2) && (~exist('OCT_TF_shift','var') || ~OCT_TF_shift)
    % if all(sino_params(1,:)==0)
    Kz_slice = 50;
...
% n0 = n_imm;
%=========================NEW
if exist('lambda','var') && (numel(n_imm) == size(sino_params,2))
    n0 = unique(n_imm(round(sino_params(3,:), 10) == lambda));
else
    n0 = n_imm;
end
%=========================
...
for ii=1:2 % repeat to correct subplot sizes
    ...
    subplot(242); imagesc(log10(logadd +abs(KOi_xz'))); axis square; %axis image
                  ...
                  colormap jet
    [nz, nx] = size(rec_zx);
    % subplot(243); imagesc(real(rec_zx));
    ax(1) = subplot(243); imagesc(real(rec_zx));
                  % colormap(ax(1), map);
                  axis image; %colorbar
                  hold on
    ...
    [ny, nx] = size(rec_yx);
    % subplot(247); imagesc(real(rec_yx)); 
    ax(2) = subplot(247); imagesc(real(rec_yx));
                  axis image; %colorbar
                  hold on
    ...
    % 				  title(sprintf('cz=%d',cz)) 
                  caxis(clims)
                  % colormap(ax(2), map);
                  colormap(ax(1), map); colormap(ax(2), map);
    if ~isempty(OBJ)
...
if exist('FileName','var') && exist('PathName','var')  
    ...
    pause(0.2); REC_plots_PO; pause(0.2); saveas(gcf, [PathName 'Crossect-' FileName '.png']);
end
