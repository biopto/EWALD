...
% 2019-2021 Wojciech Krauze, Paweł Ossowski, Marcin Sylwestrzak
% -------------------------------------------------------------------------
function [RECON,dxo, nGPi, N_Kspace_xy_padded, KO,KOi, RMAEtab,RRMSEtab,RMADtab,RRMSDtab] = ...
			FDT_cuda(SINOamp,SINOph, sino_params, thetay, ... % sinogram
						n_imm,dx_projection, ... % optical system
						geometry,Approx,interpFp,Ramp,do_NNC, ... % solver approximations
						projection_padding_xy, Kspace_padding, N_projection_padded, Kspace_oversampling_z, max_matrix_size,... % Fourier space sampling 
						nGPi,epsi,relaxGP,relaxM, ... % Gerchberg-Papoulis iterations
						init_obj,object_support,n_obj, ... % Gerchberg-Papoulis object constraints/reference
						plots, Fpmask, nGPI_CUDA)
...
% object_support               - {(Nx,Nx,Nx)|empty:[]} 3D matrix (x,y,z) of the object spatial support, if known
...
relaxLaR = 0;%0.99;%1.00, 0.99(best with N_Kspace_z_padded_upsampled > N_Kspace_xy_padded), 0:disable !!!!
...
if length(unique(size(object_support))) > 1
    error('`mask_obj` must be a cube with equal dimensions.')
elseif ~isempty(object_support) && size(object_support,1)~=Nx
...
fprintf('Max angle: %0.2f\n', max(asind(kxy./kn)));
fprintf('Min angle: %0.2f\n', min(asind(kxy./kn)));
...
if N_projection_padded<Nx
	error('Padded projection size must be equal or larger than original size: NP>=Nx.');
end
dkP = 1/(dx_projection*N_projection_padded); % frequency sample (padded projection)
...
N_Kspace_xy_padded = min(max_matrix_size,round(OTF_diameter/dkP /2)*2);
N_Kspace_z_padded_upsampled = round(Kspace_oversampling_z*N_Kspace_xy_padded/2)*2;

% save uzupelnienie_KO_835nm_UDHS.mat dkP dx_projection N_projection_padded

if N_Kspace_xy_padded==0; error('N_Kspace_xy_padded==0. Wrong units?'); end
if max_matrix_size < round(OTF_diameter/dkP /2)*2
    warning('K-space array size has been reduced (max_matrix_size) due to the RAM/VRAM limit.')
end

dkPz = dkP/(N_Kspace_z_padded_upsampled/N_Kspace_xy_padded);

% projection spectrum plane
if strcmp(geometry,'fixed')
	NF = N_Kspace_xy_padded; % must contain shadows of all shifted Ewald spheres
    % additionally upsample projection spectra for inpterpFp:
	NF = NF*q; N_projection_padded = N_projection_padded*q; dkP = dkP/q;
elseif strcmp(geometry,'facing')
	NF = N_projection_padded; % original projection band (upsampled for NP>Nx)
end
...
disp('Assembling K space from projection spectra ...')
% K-space (FT of scattering potential):
KO = single(zeros(N_Kspace_xy_padded,N_Kspace_xy_padded,N_Kspace_z_padded_upsampled)); 
...
if all(sino_params(5,:)==2)      
    ...
    Kz_slice = N_Kspace_z_padded_upsampled/2+1;
end
...
for j=1:nproj  % main loop for K-space generation
	...
	Up = padarray(Up, [(N_projection_padded-Nx)/2 (N_projection_padded-Ny)/2], 0);% abs(Up) should be >=0, as SINOph; angle(Up) should oscilate around 0
	Fpm = round(imresize(single(Fpmask(:,:,j)),[N_projection_padded N_projection_padded]));
	...
	if isempty(thetay) % LAT
		if N_projection_padded<NF && N_projection_padded>=2*NA(j)/lambda_all(j)/dkP-2% zip=1 (2*NA < projection band < 4*NA); rf = NA/lambda/dkP
    ...
    Kx = Kx/dkP/q+ceil((N_Kspace_xy_padded+1)/2); % N_Kspace_xy_padded / 2 + 1 corresponds to position of the DC term
    Ky = Ky/dkP/q+ceil((N_Kspace_xy_padded+1)/2);
    Kz = Kz/dkPz+ceil((N_Kspace_z_padded_upsampled+1)/2);
    % discard frequencies exceeding N_Kspace_xy_padded x N_Kspace_xy_padded x N_Kspace_z_padded_upsampled 
    % array subscripts
    iiF( Kx(iiF)<1 | Kx(iiF)>N_Kspace_xy_padded ) = [];
    iiF( Ky(iiF)<1 | Ky(iiF)>N_Kspace_xy_padded ) = [];
    iiF( Kz(iiF)<1 | Kz(iiF)>N_Kspace_z_padded_upsampled ) = [];
    ...
    iiK = (Kz-1)*N_Kspace_xy_padded^2 + (Ky-1)*N_Kspace_xy_padded + Kx; 
...
if ~strcmp(interpFp,'none')
	N_projection_padded=N_projection_padded/q;dkP=dkP*q;% back to actual values
end
if ~Ramp	
...
% O(Kx,Ky,Kz) --> n(x,y,z)
dxo = dx_projection*N_projection_padded/N_Kspace_xy_padded;% sample size in reconstruction space (NP/N_Kspace_xy_padded: how many more dk samples in K)


%=================================temp
% Save KO for Section 2 in paper:
% KO_filename = 'KO_fantom_835nm_181ang.mat';
% KO_filename = 'KO_tkanka_UDHS.mat';
% KO_filename = 'KO_tkanka_71wavl_181ang.mat';
% KO_filename = 'KO_kulka_71wavl_12ang.mat';
% save(KO_filename, 'KO', 'EW', 'nproj', 'OTF_diameter', 'dx_projection', 'N_projection_padded', 'sino_params', 'dxo','-v7.3');
%=================================


if ~isempty(init_obj)
	disp('Signal initialization from init_obj ...')	
	if any(isnan(init_obj)); error('NaNs detected in `init_obj`.'); end
	if size(init_obj)==[N_Kspace_xy_padded,N_Kspace_xy_padded,N_Kspace_z_padded_upsampled]% (previous n_rec from another call of D3_FDT)
		n_rec = init_obj;
        if objshift; n_rec = ifftshift(n_rec); end% mimic shape from ifftn
	elseif size(init_obj)==[Nx,Nx,Nx]% (x_tv)
        init_obj = padarray(init_obj, [1 1 1]*(N_projection_padded-Nx)/2, n_imm); % pad same way as projections (n_imm!)
		init_obj = ndresize(init_obj, [N_Kspace_xy_padded N_Kspace_xy_padded N_Kspace_xy_padded], 'linear'); % match spectrum padding
		init_obj = padarray(init_obj, [0 0 (N_Kspace_z_padded_upsampled-N_Kspace_xy_padded)/2], n_imm); % match padding from spectrum oversampling in Z
		n_rec = init_obj;		
        if objshift; n_rec = ifftshift(n_rec); end% mimic shape from ifftn
	else
		error('`init_obj` must be of size (x,y,z)=[N_Kspace_xy_padded, N_Kspace_xy_padded, N_Kspace_z_padded_upsampled] or [Nx,Nx,Nx].')
	end
else
	disp('Direct inversion ...')
    n_rec = ifftn(ifftshift(KO))./(dxo^3);
...
if ~isempty(object_support)
	object_support = padarray(object_support, [1 1 1]*(N_projection_padded-Nx)/2, 0);% pad same way as projections (with zeros)
	object_support = ndresize(object_support, [N_Kspace_xy_padded N_Kspace_xy_padded N_Kspace_xy_padded], 'linear'); % match spectrum padding
	object_support = padarray(object_support, [0 0 (N_Kspace_z_padded_upsampled-N_Kspace_xy_padded)/2], false);% match padding from spectrum oversampling in Z
	if objshift; object_support = ifftshift(object_support); end% mimic current shape of n_rec
end
...
if contains(plots,'3')
    ...
    n_rec_0_temp = ndcrop(n_rec_0,[N_Kspace_xy_padded,N_Kspace_xy_padded,N_Kspace_xy_padded]);
    n_rec_0_temp_xz = real(squeeze(n_rec_0_temp(:,end/2+1,:)).');
    n_rec_0_temp_xy = real(squeeze(n_rec_0_temp(:,:,end/2+1)).');
    
    if ~isempty(object_support)
        if objshift
            object_support_temp = ndcrop(fftshift(object_support),[N_Kspace_xy_padded,N_Kspace_xy_padded,N_Kspace_xy_padded]);
        else
            object_support_temp = ndcrop(object_support,[N_Kspace_xy_padded,N_Kspace_xy_padded,N_Kspace_xy_padded]);
        end
...
block = subarray_yx(true(1,round(N_Kspace_xy_padded/projection_padding_xy/2)*2),[1,N_Kspace_xy_padded]);
blockz = subarray_yx(true(1,round(N_Kspace_xy_padded/projection_padding_xy/2)*2),[1,N_Kspace_z_padded_upsampled]);
...
if ~isempty(n_obj)
    ...
    n_obj_0 = padarray(n_obj, [1 1 1]*(N_projection_padded-Nx)/2, n_imm); % pad same way as projections (n_imm!)
    % match spectrum padding:
    n_obj_0 = ndresize(n_obj_0, [N_Kspace_xy_padded N_Kspace_xy_padded N_Kspace_xy_padded], 'linear');
    % match padding from spectrum oversampling in Z:
    n_obj_0 = padarray(n_obj_0, [0 0 (N_Kspace_z_padded_upsampled-N_Kspace_xy_padded)/2], n_imm); 
...
% GP (GPSC) iterations begins here:
if nGPI_CUDA  % GP/GPSC on GPU (CUDA)
    
    if nGPi>0
        time1 = tic;
        disp('Fourier iterations on CUDA ...'); 
        
        % Note: relaxed suppression (relaxLaR>0) of reconstruction padding from Fourier 
        % oversampling is not included in iteration algorithm on GPU (CUDA)!
        
        %================================
        % Set parameters directly for CUDA
        % relaxM = 1; % do not change it!
        % if ~isempty(object_support); nGPi = 30; end
        %================================
        ...
        % fullpathToHeader = 'E:\User-Marcin\TT2\CUDA\12\CUDAprocessing\CUDAprocessing\CUDAprocessing.h';
        % fullpathToDll    = 'E:\User-Marcin\TT2\CUDA\12\CUDAprocessing\x64\Debug\CUDAprocessing.dll';

        [lib_folder_path, ~, ~] = fileparts(mfilename('fullpath'));
        CUDApath = fullfile(lib_folder_path, '\GP_CUDA\CUDAprocessing');

        loadlibrary(fullfile(CUDApath, 'CUDAprocessing.dll'), fullfile(CUDApath, 'CUDAprocessing.h'));
        libisloaded CUDAprocessing;

        error = calllib('CUDAprocessing','cudaInitDevice'); 
        checkError(error);

        sizes = size(n_rec);

        error = calllib('CUDAprocessing','setDims_n_rec', sizes(2), sizes(1), sizes(3));
        checkError(error);

        error = calllib('CUDAprocessing','setParams',do_TC, do_NNC, n_imm, relaxM, betaM, mean(kn), objshift, dxo, nGPi);
        checkError(error);

        error = calllib('CUDAprocessing','sendData_n_rec', convertComplexMatrixForC(n_rec));
        checkError(error);

        error = calllib('CUDAprocessing','sendData_objectSupport', convertFloatMatrixForC(object_support));
        checkError(error);

        EW_8bit1D = int8(convertFloatMatrixForC(fftshift(EW)));
        elements = sum(EW_8bit1D);

        error = calllib('CUDAprocessing','sendData_KO_Re_Im_and_EW8bit', convertComplexMatrixForC(fftshift(KO)), EW_8bit1D, elements);
        checkError(error);

        error = calllib('CUDAprocessing','cudaProcessing');
        checkError(error);

        p = libpointer('singlePtr', 1:2*sizes(2)*sizes(1)*sizes(3));

        error = calllib('CUDAprocessing','takeComplexData', p);
        checkError(error);

        n_rec = convertMatrixFromCToMatlab(p.Value, sizes(1), sizes(2), sizes(3)); 

        unloadlibrary CUDAprocessing

        clear sizes EW_8bit1D p 
...
else % use standard CPU impementation of GP/GPSC
    
    % replace 3D array with lists of occupied indices and values:
    EWi = find(EW>0); EKO = KO(EWi); clear KO;
    
    % relaxed suppression of reconstruction padding from Fourier oversampling
    % (not included in GP/GPSC algorithm on GPU (nGPi_CUDA=1)) 
    if relaxLaR>0
        cover_out = subarray_3d(single(ones(round([N_Kspace_xy_padded/projection_padding_xy, N_Kspace_xy_padded /... 
                                              projection_padding_xy, N_Kspace_xy_padded/projection_padding_xy]/2)*2)), ...
                                              [N_Kspace_xy_padded N_Kspace_xy_padded N_Kspace_z_padded_upsampled]);% LaR(best:Shepp80?)
    ...
    for gpi = 1:nGPi
        ...
        % O(Kx,Ky,Kz) --> n(x,y,z)
        n_rec = ifftn(ifftshift(KOi))./(dxo^3);
...
%     t_c_MS4 = toc(timeMS4);
%     fprintf('Iterations on CPU (MS): '); show_time(t_c_MS4);
...
end % end of nGPI_cuda

if nGPi>0
    time1 = toc(time1);
    fprintf('GP iterations '); show_time(time1);
end 
